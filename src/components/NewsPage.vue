  ...
  <div class="news-page container py-5">
    ...
    <ul class="nav nav-pills justify-content-center mb-5 news-tabs">
      <li class="nav-item" v-for="tab in tabs" :key="tab">
        <button class="nav-link" :class="{ active: currentTab === tab }" @click="currentTab = tab">{{ tab }}</button>
      </li>
    </ul>
...
// 后端新闻字段：{ id, title, type, date, summary }；type 为中文分类
const newsData = ref([
  { id: 1, type: '公司新闻', title: '鸿瑞办公成功为省市区政府办公设备指定供应商', date: '2023-03-18', excerpt: '凭借稳定的供货与高质量的售后服务，公司中标并华为市内多家单位提供设备...' },
  { id: 2, type: '公司新闻', title:'鸿瑞办公正式入驻京东慧采业务板块', date: '2026-01-10', excerpt: '2026年1月10日，鸿瑞办公京东慧采业务板块启动仪式正式开启.....'},
  { id: 5, type: '行业资讯', title:'擎启“鸿”图 智联未来 柯尼卡美能达旗下数码复合机完成与鸿蒙系统的深度适配', date: '2026-01-03', excerpt: '随着国家国产操作系统技术攻坚与生态建设顶层战略的加速落地，企业国产化....'},
  { id: 3, type: '行业资讯', title: '2025年办公设备行业数字化转型趋势分析', date: '2025-10-12', excerpt: '随着智能办公的普及，传统复印机正向云端集成终端设备转型...' },
  { id: 4, type: '公司新闻', title:'热烈祝贺鸿瑞办公2024-2025年终总结大会圆满进行', date: '2025-01-25', excerpt: '在此次大会上，公司领导与各部门代表共同回顾了过去一年的工作成果与挑战，展望了未来发展的战略.....'},
])

const tabs = computed(() => {
  const set = new Set(newsData.value.map((i) => i.type).filter(Boolean))
  return ['全部', ...set]
})

const filteredNews = computed(() => {
  if (currentTab.value === '全部') return newsData.value
  return newsData.value.filter((item) => item.type === currentTab.value)
})
...
onMounted(async () => {
  try {
    const data = await getNews({ page: 1, size: 100 })
    const records = (data && data.records) || []
    if (records.length) {
      newsData.value = records.map((n) => ({
        id: n.id,
        type: n.type || '公司新闻',
        title: n.title,
        date: n.date,
